...
class GitAnalyzer:
    ...
    def analyze_repository(self, repo_path: Path, progress_callback: Optional[ProgressCallback] = None) -> GitIntelligence:
        """
    ...
    def extract_commit_history(self, repo_path: Path) -> List[CommitInfo]:
        """Extract commit history with metadata"""
        tracker = GitGhostTracker(repo_path)
        return tracker._extract_all_commits()

    def extract_developer_profiles(self, repo_path: Path) -> List[DeveloperInfo]:
        """Build developer profiles from commit history"""
        intelligence = self.analyze_repository(repo_path)
        return intelligence.developers

    def analyze_change_patterns(self, repo_path: Path) -> ChangePatterns:
