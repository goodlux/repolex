...
class DocumentationExporter(BaseExporter):
    ...
    async def export_docs(
        ...
        try:
            ...
            functions = await self._extract_functions(org_repo, release)
            modules = await self._extract_modules(org_repo, release)
            repository_info = await self._extract_repository_info(org_repo, release)
            ...
            if format == 'mdx':
                generated_files = await self._generate_mdx_docs(
                    doc_structure, output_dir, template, progress_callback
                )
            elif format == 'html':
                generated_files = await self._generate_html_docs(
                    doc_structure, output_dir, template, progress_callback
                )
            elif format == 'markdown':
                generated_files = await self._generate_markdown_docs(
                    doc_structure, output_dir, template, progress_callback
                )
            ...
            await self._copy_assets(output_dir, format, template)
    ...
    async def _extract_functions(self, org_repo: str, release: str) -> List[FunctionInfo]:
        ...
        SELECT ?function ?name ?signature ?docstring ?file_path ?start_line ?end_line WHERE {{
            GRAPH <http://codedoc.org/repo/{org}/{repo}/functions/implementations> {{
            ...
            GRAPH <http://codedoc.org/repo/{org}/{repo}/functions/stable> {{
        ...
        results = await self.oxigraph_client.query_sparql(query)
        functions = []
    ...
    async def _extract_modules(self, org_repo: str, release: str) -> List[Dict[str, Any]]:
    ...
    async def _extract_repository_info(self, org_repo: str, release: str) -> Dict[str, Any]:
...
'''
    ...
    async def _copy_assets(self, output_dir: Path, format: str, template: Optional[str]):
