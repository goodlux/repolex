...
class FunctionSearchEngine:
    ...
    async def search_functions(
        ...
        try:
            ...
            candidates = await self._extract_candidate_functions(
    ...
    async def _extract_candidate_functions(
        self,
        ...
        if repo_filter:
            ...
            query_parts.append(f"  GRAPH <http://codedoc.org/repo/{org}/{repo}/functions/implementations> {{")
        else:
        ...
        if repo_filter:
            org, repo = repo_filter.split('/')
            query_parts.append(f"  GRAPH <http://codedoc.org/repo/{org}/{repo}/functions/stable> {{")
        else:
        ...
        # Execute query
        results = await self.oxigraph_client.query_sparql(sparql_query)
