...
from ..models.exceptions import (
    CodeDocError, GitError, StorageError, ValidationError, SecurityError
)
...
logger = logging.getLogger("codedoc.repository_vault")
...
class PacManRepositoryVault:
    ...
    - 🌟 Perfect maze organization (~/.codedoc/repos/)
    ...
    def __init__(self, base_storage_path: Optional[Path] = None):
        ...
        self.base_path = base_storage_path or Path.home() / ".codedoc" / "repos"
    ...
    async def chomp_repository(
        ...
        if progress_callback:
            await progress_callback(5.0, f"🟡 PAC-MAN starting to chomp {org_repo}...")
        ...
        try:
            ...
            if repo_path.exists():
                ...
                return await self._analyze_existing_repository(org_repo, progress_callback)
            ...
            if progress_callback:
                await progress_callback(15.0, f"🏗️ Created organization maze for {org}...")
            ...
            if progress_callback:
                await progress_callback(25.0, f"🔗 Cloning repository from GitHub...")
            
            try:
                # Use git client to clone
                await self.git_client.clone_repository(
                    github_url, 
                    repo_path,
            ...
            if progress_callback:
                await progress_callback(75.0, f"🍒 Discovering releases and tags...")
            
            # Discover all the cherry bonuses (tags/releases)! 🍒
            releases = await self._discover_repository_cherries(repo_path)
            
            if progress_callback:
                await progress_callback(85.0, f"📊 Analyzing repository structure...")
            
            # Analyze the repository maze structure
            repo_info = await self._analyze_repository_structure(org_repo, repo_path, releases)
            
            # Update vault statistics
            self.stats.maze_size += self._calculate_repository_size(repo_path)
            
            if progress_callback:
                await progress_callback(95.0, f"💾 Saving repository metadata...")
            
            # Save metadata to vault
            await self._save_repository_metadata(org_repo, repo_info)
            
            if progress_callback:
                await progress_callback(100.0, f"🟡 WAKA WAKA! {org_repo} successfully chomped!")
            ...
            repo_path = self.base_path / org / repo
            if repo_path.exists():
                await self._cleanup_failed_repository(repo_path)
            
            raise
    
    async def release_repository_ghosts(
        self, 
        org_repo: str, 
        force: bool = False,
        progress_callback: Optional[ProgressCallback] = None
        ...
        if progress_callback:
            await progress_callback(10.0, f"👻 Preparing ghost release for {org_repo}...")
        ...
        try:
            ...
            if progress_callback:
                await progress_callback(30.0, f"👻 Releasing repository ghosts...")
            ...
            if progress_callback:
                await progress_callback(70.0, f"👻 Cleaning up metadata...")
            
            # Clean up metadata
            await self._remove_repository_metadata(org_repo)
            
            if progress_callback:
                await progress_callback(90.0, f"👻 Final ghost cleanup...")
            ...
            if progress_callback:
                await progress_callback(100.0, f"👻 All ghosts released from {org_repo}!")
    ...
    async def survey_vault_dots(self) -> List[RepoInfo]:
        """
        ...
        for org_path in self.base_path.iterdir():
            ...
            for repo_path in org_path.iterdir():
                ...
                try:
                    ...
                    repo_info = await self._quick_analyze_repository_dot(org_repo, repo_path)
    ...
    async def examine_repository_dot(self, org_repo: str) -> RepoDetails:
        """
        ...
        try:
            ...
            # Get git information
            git_info = await self._analyze_git_information(repo_path)
            
            # Get file structure information
            file_stats = await self._analyze_file_structure(repo_path)
            
            # Load stored metadata if available
            metadata = await self._load_repository_metadata(org_repo)
    ...
    async def refresh_repository_dots(
        self, 
        org_repo: str,
        progress_callback: Optional[ProgressCallback] = None
    ) -> UpdateResult:
        ...
        if progress_callback:
            await progress_callback(10.0, f"🔄 Starting refresh for {org_repo}...")
        
        try:
            # Get old release count for comparison
            old_releases = await self._discover_repository_cherries(repo_path)
            old_release_count = len(old_releases)
            
            if progress_callback:
                await progress_callback(30.0, f"📡 Pulling latest changes from GitHub...")
            
            # Pull latest changes
            pull_result = await self.git_client.pull_repository(
                repo_path,
                progress_callback=self._create_git_progress_wrapper(progress_callback, 30.0, 70.0)
            )
            
            if progress_callback:
                await progress_callback(75.0, f"🍒 Discovering new cherry releases...")
            
            # Discover new releases
            new_releases = await self._discover_repository_cherries(repo_path)
            ...
            if progress_callback:
                await progress_callback(90.0, f"💾 Updating repository metadata...")
            
            # Update metadata
            repo_info = await self._analyze_repository_structure(org_repo, repo_path, new_releases)
            await self._save_repository_metadata(org_repo, repo_info)
            
            if progress_callback:
                await progress_callback(100.0, f"🟡 Repository refresh complete!")
    ...
    async def checkout_version_dot(
        self, 
        org_repo: str, 
        version: str,
        progress_callback: Optional[ProgressCallback] = None
        ...
        if progress_callback:
            await progress_callback(10.0, f"🔵 Preparing version checkout for {version}...")
        ...
        try:
            ...
            if progress_callback:
                await progress_callback(30.0, f"🔄 Creating version-specific checkout...")
            
            # Create version-specific directory
            version_path.mkdir(exist_ok=True)
            
            # Use git worktree or clone approach for version isolation
            await self.git_client.checkout_version(
                repo_path, 
                version, 
                version_path,
            ...
            if progress_callback:
                await progress_callback(100.0, f"🔵 Version dot {version} ready!")
    ...
    def _create_git_progress_wrapper(
        ...
        async def git_progress_wrapper(percent: float, message: str):
            ...
            await progress_callback(mapped_percent, f"🔗 {message}")
        
        return git_progress_wrapper
    
    async def _discover_repository_cherries(self, repo_path: Path) -> List[ReleaseInfo]:
        """Discover all cherry bonuses (tags/releases) in repository"""
        try:
            releases = await self.git_client.discover_releases(repo_path)
    ...
    async def _analyze_existing_repository(
        self, 
        org_repo: str, 
        progress_callback: Optional[ProgressCallback]
    ) -> RepoResult:
        ...
        if progress_callback:
            await progress_callback(50.0, f"🍒 Repository already in vault - analyzing...")
        ...
        if progress_callback:
            await progress_callback(100.0, f"🟡 Repository analysis complete!")
    ...
    async def _analyze_git_information(self, repo_path: Path) -> Dict[str, Any]:
        """Analyze git information for repository"""
        try:
            git_info = await self.git_client.get_repository_info(repo_path)
            return git_info
        except Exception as e:
            logger.warning(f"👻 Ghost encountered analyzing git info: {e}")
            return {}
    
    async def _quick_analyze_repository_dot(self, org_repo: str, repo_path: Path) -> RepoInfo:
    ...
    async def _save_repository_metadata(self, org_repo: str, repo_info: RepoInfo):
        """Save repository metadata to vault"""
        try:
            org, repo = org_repo.split('/')
            metadata_path = self.base_path / org / repo / ".codedoc_metadata.json"
    ...
    async def _load_repository_metadata(self, org_repo: str) -> Optional[Dict[str, Any]]:
    ...
    async def _remove_repository_metadata(self, org_repo: str):
    ...
    async def _cleanup_failed_repository(self, repo_path: Path):
...
def create_pacman_repository_vault(storage_path: Optional[Path] = None) -> PacManRepositoryVault:
    ...
    Args:
        storage_path: Custom storage path (defaults to ~/.codedoc/repos)
