...
class GraphManager:
    ...
    def __init__(self, storage_root: Optional[Path] = None):
        ...
        print("🟡 PAC-MAN Graph Manager initialized and ready to chomp semantic dots!")
        print(f"🗄️  Storage: {self.storage_root}")
        print("WAKA WAKA WAKA!")
    
    async def add_graphs(self, org_repo: str, release: Optional[str] = None, progress_callback: Optional[ProgressCallback] = None) -> ProcessingResult:
        """
        ...
        try:
            ...
            if not repo_path.exists():
                raise ValidationError(
                    ...
                    suggestions=[f"Use 'codedoc repo add {org_repo}' first to clone the repository"]
                )
            
            # Step 2: Determine which release to process
            if not release:
                release = await self._get_latest_release(repo_path)
                if not release:
            ...
            await self._validate_release_exists(repo_path, release)
            ...
            existing_graphs = await self._check_existing_graphs(org_repo, release)
            if existing_graphs:
                raise ValidationError(
                    f"Graphs already exist for {org_repo} {release}",
                    suggestions=[
                        f"Use 'codedoc graph update {org_repo} {release}' to rebuild",
                        f"Use 'codedoc graph remove {org_repo} {release}' to remove first"
                    ]
                )
            ...
            git_data = await self._analyze_git_intelligence(repo_path, progress_callback)
            ...
            abc_events = await self._generate_abc_events(org_repo, release, ast_data, git_data, progress_callback)
            ...
            graphs_created = await self._build_all_graphs(org_repo, release, ast_data, git_data, abc_events, progress_callback)
            ...
            triples_stored = await self._store_semantic_data(org_repo, release, graphs_created, progress_callback)
            ...
            await self._update_processing_metadata(org_repo, release, stats)
    ...
    async def remove_graphs(self, org_repo: str, release: Optional[str] = None, force: bool = False) -> bool:
        """
        ...
        try:
            if release:
                ...
                graphs_removed = await self._remove_release_graphs(org_repo, release)
                ...
                graphs_removed = await self._remove_all_repository_graphs(org_repo)
    ...
    async def list_graphs(self, org_repo: Optional[str] = None) -> List[GraphInfo]:
        """🟡 PAC-MAN surveys the semantic maze - list all graphs."""
        
        try:
            if org_repo:
                validate_org_repo(org_repo)
                graphs = await self.oxigraph.list_repository_graphs(org_repo)
            else:
                graphs = await self.oxigraph.list_all_graphs()
            
            graph_info_list = []
            for graph_uri in graphs:
                info = await self._get_graph_info(graph_uri)
                graph_info_list.append(info)
            
            print(f"🟡 PAC-MAN found {len(graph_info_list)} semantic maze levels!")
    ...
    async def show_graphs(self, org_repo: str, release: Optional[str] = None) -> GraphDetails:
        ...
        try:
            ...
            graphs = await self._get_repository_graphs(org_repo, release)
            
            if not graphs:
                raise ValidationError(
                    f"No graphs found for {org_repo}" + (f" {release}" if release else ""),
                    suggestions=[
                        f"Use 'codedoc graph add {org_repo}" + (f" {release}" if release else "") + "' to create graphs"
                    ]
                )
            ...
            for graph_uri in graphs:
                info = await self._get_detailed_graph_info(graph_uri)
    ...
    async def update_graphs(self, org_repo: str, release: Optional[str] = None, progress_callback: Optional[ProgressCallback] = None) -> ProcessingResult:
        """
        ...
        try:
            ...
            if not existing_graphs:
                raise ValidationError(
                    ...
                    suggestions=[f"Use 'codedoc graph add {org_repo}' to create graphs first"]
                )
            ...
            await self._nuclear_clear_implementation_graphs(org_repo, release)
            ...
            result = await self.add_graphs(org_repo, release, progress_callback)
    ...
    async def _get_latest_release(self, repo_path: Path) -> Optional[str]:
    ...
    async def _validate_release_exists(self, repo_path: Path, release: str):
    ...
    async def _check_existing_graphs(self, org_repo: str, release: Optional[str]) -> List[str]:
    ...
    async def _analyze_git_intelligence(self, repo_path: Path, progress_callback: Optional[ProgressCallback] = None):
        ...
        return await self.git_analyzer.analyze_repository(repo_path, progress_callback)
    ...
    async def _store_semantic_data(self, org_repo: str, release: str, graphs_created: List[str], progress_callback: Optional[ProgressCallback] = None) -> int:
        ...
        return await self.oxigraph.store_all_graphs(org_repo, release, graphs_created, progress_callback)
    
    async def _update_processing_metadata(self, org_repo: str, release: str, stats: SemanticProcessingStats):
        """Update processing metadata for tracking."""
        repo_path = self._get_repo_path(org_repo)
        metadata_file = repo_path / ".codedoc" / "processing_metadata.json"
    ...
    async def _remove_release_graphs(self, org_repo: str, release: str) -> int:
        """Remove graphs for specific release only."""
        return await self.oxigraph.remove_release_graphs(org_repo, release)
    
    async def _remove_all_repository_graphs(self, org_repo: str) -> int:
        """Remove ALL graphs for repository."""
        return await self.oxigraph.remove_all_repository_graphs(org_repo)
    
    async def _nuclear_clear_implementation_graphs(self, org_repo: str, release: Optional[str]):
        """Nuclear clear implementation graphs while preserving stable identities."""
        await self.oxigraph.nuclear_clear_implementation_graphs(org_repo, release)
    
    async def _get_graph_info(self, graph_uri: str) -> GraphInfo:
        """Get basic information about a graph."""
        triple_count = await self.oxigraph.count_triples_in_graph(graph_uri)
    ...
    async def _get_detailed_graph_info(self, graph_uri: str):
        """Get detailed information about a graph."""
        return await self.oxigraph.get_detailed_graph_info(graph_uri)
    
    async def _get_repository_graphs(self, org_repo: str, release: Optional[str]) -> List[str]:
...
async def chomp_repository_semantics(org_repo: str, release: Optional[str] = None, **kwargs) -> ProcessingResult:
    """
    ...
    return await manager.add_graphs(org_repo, release, **kwargs)

async def power_pellet_rebuild(org_repo: str, release: Optional[str] = None, **kwargs) -> ProcessingResult:
    """
    ...
    """
    manager = GraphManager()
    return await manager.update_graphs(org_repo, release, **kwargs)
